...
fn main() {
    ...
    let base = rand::thread_rng().gen_range(256..2 << 32);
    for si in &s {
        let rh = mylib::RollingHash::from_chars(si, base);
        for j in 0..si.len() {
            *map.entry(rh.get_hash(0, j + 1)).or_insert(0) += 1;
        }
    }

    let ans = map
        .values()
        .filter_map(|&x| if x >= 2 { Some(x * (x - 1) / 2) } else { None })
        .sum::<usize>();
