...
fn main() {
    // 解説 AC
    input! {
        h: usize, w: usize, n: usize, hh: usize, ww: usize,
        a: [[Usize1; w]; h],
    }

    let mut min_x = vec![usize::MAX; n];
    let mut min_y = vec![usize::MAX; n];
    let mut max_x = vec![usize::MIN; n];
    let mut max_y = vec![usize::MIN; n];

    for i in 1..=h {
        for j in 1..=w {
            let idx = a[i-1][j-1];
            min_x[idx] = min_x[idx].min(i);
            min_y[idx] = min_y[idx].min(j);
            max_x[idx] = max_x[idx].max(i);
            max_y[idx] = max_y[idx].max(j);
        }
    }
    ...
    for k in 0..=h - hh {
        ...
        for l in 0..=w - ww {
            let mut ans = n;
            for m in 0..n {
                if k < min_x[m] && max_x[m] <= k + hh && l < min_y[m] && max_y[m] <= l + ww {
                    ans -= 1;
                }
            }
            row.push(ans);
...
fn solve() {
    input! {
        h: usize, w: usize, _n: usize, hh: usize, ww: usize,
        a: [[usize; w]; h],
    }

    let mut set = mylib::MultiBTreeSet::from_iter(a.iter().flatten().copied());
    ...
    for k in 0..=h - hh {
        ...
        for l in 0..=w - ww {
            if l > 0 {
                for i in 0..hh {
                    subset.insert(a[k + i][l - 1]);
                    subset.remove(&a[k + i][l + ww - 1]);
                }
            }
            row.push(subset.len_unique());
        }
        println!("{}", row.iter().join(" "));
    }
}
