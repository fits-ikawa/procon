...
fn main() {
    ...
    let c = c
        .into_iter()
        .map(|row| row.into_iter().map(|char| char as u8 - b'a').collect_vec())
        .collect_vec();

    use mylib::MultiBTreeSet;

    let mut row = vec![MultiBTreeSet::new(); h];
    let mut col = vec![MultiBTreeSet::new(); w];
    ...
    loop {
        let mut remove_from_col = hashmap! {};
        for x in 0..h {
            if row[x].len_unique() == 1 {
                let char = row[x].first().copied().unwrap();
                if row[x].count(&char) >= 2 {
                    let value = remove_from_col.entry(char).or_insert(0);
                    *value += 1;
                    row[x].clear();
                }
            }
        ...
        let mut remove_from_row = hashmap! {};
        for y in 0..w {
            if col[y].len_unique() == 1 {
                let char = col[y].first().copied().unwrap();
                if col[y].count(&char) >= 2 {
                    let value = remove_from_row.entry(char).or_insert(0);
                    *value += 1;
                    col[y].clear();
                }
            }
        ...
        if remove_from_col.is_empty() && remove_from_row.is_empty() {
            break;
        }

        for (k, v) in remove_from_col {
            for y in 0..w {
                col[y].remove_n(&k, v);
            }
        }

        for (k, v) in remove_from_row {
            for x in 0..h {
                row[x].remove_n(&k, v);
            }
        }
    ...
    let ans = row.iter().map(|r| r.len()).sum::<usize>();
