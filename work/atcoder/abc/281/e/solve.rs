...
fn main() {
    ...
    let b = a[..m].iter().sorted().copied().collect_vec();

    let mut left = mylib::MultiBTreeSet::new();
    let mut right = mylib::MultiBTreeSet::new();
    let mut sum = 0;
    ...
    for i in 1..n - m + 1 {
        ...
        if come < right.first().copied().unwrap_or(usize::MAX) {
            left.insert(come);
            sum += come;
        } else {
            right.insert(come);
        ...
        if left.len() > k {
            let mv = left.pop_last().unwrap();
            right.insert(mv);
            sum -= mv;
        } else if left.len() < k {
            let mv = right.pop_first().unwrap();
            left.insert(mv);
            sum += mv;
        }
    }
