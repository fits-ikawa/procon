...
fn main() {
    ...
    let mut acc2d = mylib::Cumsum2D::from_vec(p);
    acc2d.build();
    ...
    let f = |x, y| {
        ...
        acc2d.sum(0, 0, n, n) * rep_h * rep_w
            + acc2d.sum(0, 0, mod_h, n) * rep_w
            + acc2d.sum(0, 0, n, mod_w) * rep_h
            + acc2d.sum(0, 0, mod_h, mod_w)
    };

    for (a, b, c, d) in abcd {
        let ans = f(c + 1, d + 1) + f(a, b) - f(c + 1, b) - f(a, d + 1);
        println!("{}", ans);
    }
}
